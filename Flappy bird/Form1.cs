    ...
    public partial class Form1 : Form
    {
        ...
        private void ReadAndShowScore()
        {
            using (StreamReader reader = new StreamReader("Score.ini")) 
            {
                ...
                {
                    MessageBox.Show(string.Format("Felicitary ai facut scor mai mare decat {0}. Noul scor estate{1}", score, label1.Text), "Flappy Bird", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                if(score > int.Parse(label1.Text))
                {
                    MessageBox.Show(string.Format("Iti mai trebuia {0} ca sa depasesti scorul maxim de {1}", scoreDifferent, score), "Flappy Bird");
                }
                if(score == int.Parse(label1.Text))
                {
                    MessageBox.Show(string.Format("Ai facut exact {0} (scorul maxim),insearca sa-1 depasesti de data asta", score,"Flappy Bird", MessageBoxButtons.OK,MessageBoxIcon.Information));
                }
            }
        }
        ...
        private void CheckForPoint()
        {
            ...
            if (!ResetPipes | start)
            {
                if (intersect1 != Rectangle.Empty | intersect2 != Rectangle.Empty)
                {
                    ...
                    {
                        points++;
                        SoundPlayer sp = new SoundPlayer(Flappy_bird.Properties.Resources.point);
                        sp.Play();
                        inPipe = true;
                    }
                }
        ...
        private void CheckForCollision()
        {
            ...
            if (!ResetPipes | start)
            {
                ...
                {
                    SoundPlayer sp = new SoundPlayer(Flappy_bird.Properties.Resources.die);
                    sp.Play();
                    Die();
                }
            }
        ...
        private void Form1_KeyDown(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                case Keys.Space:
                    step = -5;
                    pictureBox1.Image = Resources.bg1;
                    break;
            }
        }
